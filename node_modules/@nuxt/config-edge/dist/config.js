/*!
 * @nuxt/config-edge v2.3.0-25704955.ae8dc96 (c) 2016-2018
...
const server = ({ env: env$$1 }) => ({
  https: false,
  port: env$$1.NUXT_PORT ||
    env$$1.PORT ||
    env$$1.npm_package_config_nuxt_port ||
    3000,
  host: env$$1.NUXT_HOST ||
    env$$1.HOST ||
    env$$1.npm_package_config_nuxt_host ||
    'localhost',
  socket: env$$1.UNIX_SOCKET ||
    env$$1.npm_package_config_unix_socket
});
...
function basePickBy(object, paths, predicate) {
  ...
  while (++index < length) {
    var path$$1 = paths[index],
        value = _baseGet(object, path$$1);

    if (predicate(value, path$$1)) {
      _baseSet(result, _castPath(path$$1, object), value);
    }
  }
  return result;
}
...
function basePick(object, paths) {
  return _basePickBy(object, paths, function(value, path$$1) {
    return hasIn_1(object, path$$1);
  });
}
...
function getNuxtConfig(_options) {
  ...
  options.globalName = (commonEdge.isNonEmptyString(options.globalName) && /^[a-zA-Z]+$/.test(options.globalName))
    ? options.globalName.toLowerCase()
    : `nuxt`;

  // Resolve rootDir
  options.rootDir = commonEdge.isNonEmptyString(options.rootDir) ? path.resolve(options.rootDir) : process.cwd();
  ...
  const hasSrcDir = commonEdge.isNonEmptyString(options.srcDir);
  const hasGenerateDir = commonEdge.isNonEmptyString(options.generate.dir);
  ...
  commonEdge.guardDir(options, 'rootDir', 'buildDir');
  ...
  if (hasGenerateDir) {
    ...
    // Protect rootDir against buildDir
    commonEdge.guardDir(options, 'rootDir', 'generate.dir');
  }

  if (hasSrcDir) {
    // Protect srcDir against buildDir
    commonEdge.guardDir(options, 'srcDir', 'buildDir');

    if (hasGenerateDir) {
      // Protect srcDir against generate.dir
      commonEdge.guardDir(options, 'srcDir', 'generate.dir');
    }
  }
  ...
  options.modulesDir = []
    ...
    .concat(path.join(options.nuxtDir, 'node_modules')).filter(commonEdge.isNonEmptyString)
  ...
  /* istanbul ignore if */
  if (options.dev && commonEdge.isUrl(options.build.publicPath)) {
  ...
  if (options.loadingIndicator) {
    ...
    if (!commonEdge.isPureObject(options.loadingIndicator)) {
