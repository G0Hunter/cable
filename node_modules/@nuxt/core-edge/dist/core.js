/*!
 * @nuxt/core-edge v2.3.0-25704955.ae8dc96 (c) 2016-2018
...
class ModuleContainer {
  ...
  async ready() {
    ...
    await commonEdge.sequence(this.options.modules, this.addModule.bind(this));
  ...
  extendBuild(fn) {
    this.options.build.extend = commonEdge.chainFn(this.options.build.extend, fn);
  }

  extendRoutes(fn) {
    this.options.router.extendRoutes = commonEdge.chainFn(
...
class Resolver {
  ...
  resolveAlias(path$$1) {
    if (commonEdge.startsWithRootAlias(path$$1)) {
      return path.join(this.options.rootDir, path$$1.substr(2))
    }

    if (commonEdge.startsWithSrcAlias(path$$1)) {
...
class Nuxt extends commonEdge.Hookable {
  constructor(options = {}) {
    super();
    ...
    // Add Legacy aliases
    commonEdge.defineAlias(this, this.server, ['renderRoute', 'renderAndGetWindow', 'listen']);
    commonEdge.defineAlias(this, this.resolver, ['resolveAlias', 'resolvePath']);
