/*!
 * @nuxt/builder-edge v2.3.0-25704955.ae8dc96 (c) 2016-2018
...
const devalue = _interopDefault(require('@nuxtjs/devalue'));
const commonEdge = require('@nuxt/common-edge');
...
function parent(object, path$$1) {
  return path$$1.length < 2 ? object : _baseGet(object, _baseSlice(path$$1, 0, -1));
}
...
function baseUnset(object, path$$1) {
  path$$1 = _castPath(path$$1, object);
  object = _parent(object, path$$1);
  return object == null || delete object[_toKey(last_1(path$$1))];
}
...
var omit = _flatRest(function(object, paths) {
  ...
  var isDeep = false;
  paths = _arrayMap(paths, function(path$$1) {
    path$$1 = _castPath(path$$1, object);
...
function customDefaultsAssignIn(objValue, srcValue, key, object) {
  if (objValue === undefined ||
      (eq_1(objValue, objectProto$g[key]) && !hasOwnProperty$d.call(object, key))) {
    return srcValue;
  }
  return objValue;
}
...
class Builder {
  constructor(nuxt, bundleBuilder) {
    ...
    this.globals = commonEdge.determineGlobals(nuxt.options.globalName, nuxt.options.globals);
    this.watchers = {
      files: null,
      custom: null,
      restart: null
    ...
    this.relativeToBuild = (...args) =>
      commonEdge.relativeTo(this.options.buildDir, ...args);
    ...
    // Resolve template
    this.template = this.options.build.template || '@nuxt/vue-app-edge';
  ...
  getBundleBuilder(BundleBuilder) {
    ...
    if (typeof BundleBuilder !== 'function') {
      BundleBuilder = require('@nuxt/webpack-edge').BundleBuilder;
    }
  ...
  async build() {
    ...
    await fsExtra.remove(commonEdge.r(this.options.buildDir));
    const buildDirs = [commonEdge.r(this.options.buildDir, 'components')];
    if (!this.options.dev) {
      buildDirs.push(
        commonEdge.r(this.options.buildDir, 'dist', 'client'),
        commonEdge.r(this.options.buildDir, 'dist', 'server')
      );
    }
  ...
  async generateRoutesAndFiles() {
    ...
    if (!templateVars.layouts.default) {
      await fsExtra.mkdirp(commonEdge.r(this.options.buildDir, 'layouts'));
    ...
    if (this._defaultPage) {
      templateVars.router.routes = commonEdge.createRoutes(
        ['index.vue'],
    ...
    templatesFiles = templatesFiles
      .map((file) => {
        ...
        const customPath = commonEdge.r(this.options.srcDir, 'app', file);
        const customFileExists = fsExtra.existsSync(customPath);

        return {
          src: customFileExists ? customPath : commonEdge.r(this.template.templatesDir, file),
          dst: file,
    ...
    templatesFiles = templatesFiles.concat(
      this.options.build.templates.map((t) => {
        return Object.assign(
          {
            src: commonEdge.r(this.options.srcDir, t.src || t),
    ...
    await Promise.all(
      templatesFiles.map(async ({ src, dst, options, custom }) => {
        ...
        let content;
        try {
          const templateFunction = template_1(fileContent, {
            imports: {
              serialize,
              devalue,
              hash,
          ...
          content = commonEdge.stripWhitespace(
            templateFunction(
        ...
        const _path = commonEdge.r(this.options.buildDir, dst);
  ...
  watchClient() {
    ...
    let patterns = [
      commonEdge.r(src, this.options.dir.layouts),
      commonEdge.r(src, this.options.dir.store),
      commonEdge.r(src, this.options.dir.middleware),
      commonEdge.r(src, `${this.options.dir.layouts}/*.{vue,js}`),
      commonEdge.r(src, `${this.options.dir.layouts}/**/*.{vue,js}`)
    ];
    if (this._nuxtPages) {
      patterns.push(
        commonEdge.r(src, this.options.dir.pages),
        commonEdge.r(src, `${this.options.dir.pages}/*.{vue,js}`),
        commonEdge.r(src, `${this.options.dir.pages}/**/*.{vue,js}`)
      );
    }
    patterns = map_1(patterns, upath.normalizeSafe);
    ...
    let customPatterns = concat_1(
      this.options.build.watch,
      ...values_1(omit_1(this.options.build.styleResources, ['options']))
    );
    customPatterns = map_1(uniq_1(customPatterns), upath.normalizeSafe);
  ...
  watchServer() {
    const nuxtRestartWatch = concat_1(
      this.options.serverMiddleware
        .filter(commonEdge.isString)
