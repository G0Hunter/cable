/*!
 * @nuxt/generator-edge v2.3.0-25704955.ae8dc96 (c) 2016-2018
...
class Generator {
  constructor(nuxt, builder) {
    ...
    this.distNuxtPath = path.join(
      this.distPath,
      commonEdge.isUrl(this.options.build.publicPath) ? '' : this.options.build.publicPath
    );
  }
  ...
  async initRoutes(...args) {
    ...
    if (this.options.router.mode !== 'hash') {
      try {
        generateRoutes = await commonEdge.promisifyRoute(
    ...
    let routes =
      this.options.router.mode === 'hash'
        ? ['/']
        : commonEdge.flatRoutes(this.options.router.routes);
      ...
      await Promise.all(
        routes
          ...
          .map(async ({ route, payload }) => {
            await commonEdge.waitFor(n++ * this.options.generate.interval);
  ...
  decorateWithPayloads(routes, generateRoutes) {
    ...
    generateRoutes.forEach((route) => {
      ...
      const path$$1 = commonEdge.isString(route) ? route : route.route;
      routeMap[path$$1] = {
        route: path$$1,
