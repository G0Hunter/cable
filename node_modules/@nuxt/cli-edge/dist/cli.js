/*!
 * @nuxt/cli-edge v2.3.0-25704955.ae8dc96 (c) 2016-2018
...
const parseArgs = _interopDefault(require('minimist'));
...
const start = () => Promise.resolve().then(function () { return start$2; });
const dev = () => Promise.resolve().then(function () { return dev$2; });
const build = () => Promise.resolve().then(function () { return build$2; });
const generate = () => Promise.resolve().then(function () { return generate$2; });
const help = () => Promise.resolve().then(function () { return help$2; });

const commands = /*#__PURE__*/Object.freeze({
  start: start,
  dev: dev,
  build: build,
  generate: generate,
...
const builder = () => Promise.resolve(require('@nuxt/builder-edge'));
const webpack = () => Promise.resolve(require('@nuxt/webpack-edge'));
const generator = () => Promise.resolve(require('@nuxt/generator-edge'));
const core = () => Promise.resolve(require('@nuxt/core-edge'));
...
var version = "2.3.0-25704955.ae8dc96";
...
class NuxtCommand {
  constructor({ name: name$$1, description, usage, options, run } = {}) {
    this.name = name$$1 || '';
  ...
  static async load(name$$1) {
    if (name$$1 in commands) {
      const cmd = await commands[name$$1]() // eslint-disable-line import/namespace
        .then(m => m.default);
      return NuxtCommand.from(cmd)
    } else {
      // TODO dynamic module loading
      throw new Error('Command ' + name$$1 + ' could not be loaded!')
    }
  }
  ...
  async getNuxtConfig(argv, extraOptions) {
    ...
    for (const name$$1 of Object.keys(this.options)) {
      this.options[name$$1].prepare && this.options[name$$1].prepare(this, options, argv);
    }
  ...
  _getMinimistOptions() {
    ...
    for (const name$$1 of Object.keys(this.options)) {
      const option = this.options[name$$1];
  ...
  _getHelp() {
    ...
    for (const name$$1 in this.options) {
      const option = this.options[name$$1];
...
const start$1 = {
  ...
  async run(cmd) {
    ...
    if (nuxt.options.render.ssr === true) {
      ...
      if (!fs__default.existsSync(ssrBundlePath)) {
        consola.fatal(
...
const start$2 = /*#__PURE__*/Object.freeze({
  default: start$1
});
...
const dev$2 = /*#__PURE__*/Object.freeze({
  default: dev$1
});
...
const build$2 = /*#__PURE__*/Object.freeze({
  default: build$1
});
...
const generate$2 = /*#__PURE__*/Object.freeze({
  default: generate$1
});
...
const help$2 = /*#__PURE__*/Object.freeze({
  default: help$1
});
