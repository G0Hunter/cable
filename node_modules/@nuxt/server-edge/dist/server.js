/*!
 * @nuxt/server-edge v2.3.0-25704955.ae8dc96 (c) 2016-2018
...
async function renderAndGetWindow(
  ...
  await commonEdge.timeout(new Promise((resolve) => {
    window[loadedCallback] = () => resolve(window);
  }), loadingTimeout, `Components loading in renderAndGetWindow was not completed in ${commonEdge.timeout / 1000}s`);
...
const nuxtMiddleware = ({ options, nuxt, renderRoute, resources }) => async function nuxtMiddleware(req, res, next) {
  // Get context
  const context = commonEdge.getContext(req, res);
...
const modernBrowsers = Object.keys(commonEdge.ModernBrowsers)
  .map(browser => `${browser} >= ${commonEdge.ModernBrowsers[browser]}`);
...
class Server {
  constructor(nuxt) {
    ...
    this.publicPath = commonEdge.isUrl(this.options.build.publicPath)
  ...
  async ready() {
    ...
    const { VueRenderer } = await Promise.resolve(require('@nuxt/vue-renderer-edge'));
