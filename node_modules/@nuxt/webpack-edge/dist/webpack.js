/*!
 * @nuxt/webpack-edge v2.3.0-25704955.ae8dc96 (c) 2016-2018
...
class PostcssConfig {
  ...
  loadPlugins(config) {
    ...
    if (commonEdge.isPureObject(plugins)) {
  ...
  config() {
    ...
    if (commonEdge.isPureObject(config)) {
      if (config.preset) {
...
class StyleLoader {
  ...
  normalize(loaders) {
    loaders = commonEdge.wrapArray(loaders);
  ...
  styleResource(ext) {
    ...
    if (extResource) {
      const patterns = commonEdge.wrapArray(extResource).map(p => path.resolve(this.rootDir, p));
...
class WebpackBaseConfig {
  ...
  output() {
    return {
      ...
      publicPath: commonEdge.isUrl(this.options.build.publicPath)
        ? this.options.build.publicPath
        : commonEdge.urlJoin(this.options.router.base, this.options.build.publicPath)
    }
  }
  ...
  config() {
    ...
    return cloneDeep_1(extendedConfig)
  }
}
...
class WebpackBundler {
  ...
  async build() {
    ...
    Object.keys(styleResources).forEach(async (ext) => {
      await Promise.all(commonEdge.wrapArray(styleResources[ext]).map(async (p) => {
    ...
    // Start Builds
    const runner = options.dev ? commonEdge.parallel : commonEdge.sequence;
